...
export const CategoryModal: React.FC<CategoryModalProps> = ({ isOpen, onClose }) => {
  ...
  // State for inline edit card
  const [inlineEditingId, setInlineEditingId] = useState<string | null>(null);
  const [inlineNameAr, setInlineNameAr] = useState('');
  const [inlineNameEn, setInlineNameEn] = useState('');
  const [inlineIconName, setInlineIconName] = useState('Coffee');
  const [inlineDescriptionAr, setInlineDescriptionAr] = useState('');
  ...
  const ICON_OPTIONS = [
    ...
    { name: 'Cake', label: 'حلويات 🍰', icon: Cake },
    { name: 'Cookie', label: 'مخبوزات 🍪', icon: Cookie },
    { name: 'GlassWater', label: 'عصائر 🥤', icon: GlassWater },
    { name: 'CupSoda', label: 'كوكتيل 🍹', icon: CupSoda },
    { name: 'Milk', label: 'ميلك شيك 🥛', icon: Milk },
    { name: 'IceCream', label: 'آيس كريم 🍦', icon: IceCream },
    { name: 'Croissant', label: 'فطائر 🥐', icon: Croissant },
    { name: 'Utensils', label: 'وجبات 🍽️', icon: Utensils },
    { name: 'Sparkles', label: 'مميز ✨', icon: Sparkles },
    { name: 'Heart', label: 'مفضل ❤️', icon: Heart },
  ];

  const handleStartEdit = (cat: Category) => {
    // Open inline edit for this category card
    setInlineEditingId(cat.id);
    setInlineNameAr(cat.nameAr);
    setInlineNameEn(cat.nameEn || '');
    setInlineIconName(cat.iconName || 'Coffee');
    setInlineDescriptionAr(cat.descriptionAr || '');

    // Also set top form state if needed
    setEditingId(cat.id);
  ...
  const handleSaveInlineEdit = (catId: string) => {
    if (!inlineNameAr.trim()) {
    ...
    updateCategory({
      id: catId,
      nameAr: inlineNameAr.trim(),
      nameEn: inlineNameEn.trim() || inlineNameAr.trim(),
      iconName: inlineIconName,
      descriptionAr: inlineDescriptionAr.trim()
    });
    ...
    setMsg({ type: 'success', text: 'تم تعديل القسم وتحديث القائمة بنجاح! ✓' });
  ...
  const renderIcon = (name: string, className = "w-4 h-4 text-[#00A859]") => {
    const opt = ICON_OPTIONS.find(i => i.name === name);
    if (!opt) return <Coffee className={className} />;
  ...
  return (
    <div className="fixed inset-0 z-70 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-fadeIn">
      <div id="category-modal-container" className="bg-[#1C1814] border border-[#D4A373] rounded-3xl p-5 sm:p-6 max-w-2xl w-full space-y-5 text-right text-[#FAEDCD] max-h-[90vh] overflow-y-auto scrollbar-none shadow-2xl">
        ...
        {/* ADD / EDIT FORM TOP */}
        <form id="top-category-form" onSubmit={handleSaveCategory} className="bg-[#26201B] p-4 rounded-2xl border border-[#3D332A] space-y-3.5">
            ...
            {categories.map((cat) => {
              ...
              if (isInlineEditing) {
                return (
                  <div
                    key={cat.id}
                    className="p-4 rounded-2xl bg-[#26201B] border-2 border-[#00A859] space-y-3 animate-fadeIn shadow-lg"
                  >
                    <div className="flex items-center justify-between border-b border-[#3D332A] pb-2">
                      <span className="font-bold text-xs text-[#00A859] flex items-center gap-1.5">
                        <Edit3 className="w-4 h-4" />
                        <span>جاري تعديل قسم: {cat.nameAr}</span>
                      </span>
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      <div>
                        <label className="block text-[11px] font-bold text-[#D4A373] mb-1">الاسم بالعربي *</label>
                        <input
                          type="text"
                          value={inlineNameAr}
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-[#D4A373] mb-1">الاسم بالإنجليزي</label>
                        <input
                          type="text"
                          value={inlineNameEn}
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-[#D4A373] mb-1">اختر الأيقونة / الرمز</label>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                        {ICON_OPTIONS.map((opt) => {
                          const IconComp = opt.icon;
                          const isSel = inlineIconName === opt.name;
                          return (
                            <button
                              key={opt.name}
                              type="button"
                              onClick={() => setInlineIconName(opt.name)}
                              className={`p-1.5 rounded-lg border text-[11px] font-bold flex items-center gap-1.5 cursor-pointer ${
                                isSel
                                  ? 'bg-[#00A859] text-white border-[#00A859]'
                                  : 'bg-[#181512] text-[#FAEDCD]/80 border-[#3D332A]'
                              }`}
                            >
                              <IconComp className={`w-3.5 h-3.5 ${isSel ? 'text-white' : 'text-[#00A859]'}`} />
                              <span className="truncate">{opt.label}</span>
                            </button>
                          );
                        })}
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-[#D4A373] mb-1">الوصف</label>
                      <input
                        type="text"
                        value={inlineDescriptionAr}
                        onChange={(e) => setInlineDescriptionAr(e.target.value)}
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <button
                        ...
                        className="bg-[#00A859] hover:bg-[#008A48] text-white font-bold text-xs py-2 px-4 rounded-xl flex items-center gap-1.5 transition-all cursor-pointer"
                      >
                      ...
                      <button
                        type="button"
                        onClick={handleCancelInlineEdit}
                        className="bg-[#3D332A] text-[#FAEDCD] font-bold text-xs py-2 px-3 rounded-xl cursor-pointer"
                      >
                        إلغاء
                      </button>
                    </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-[#FAEDCD]/60 mt-0.5">
                        ...
                        {cat.descriptionAr && <span className="mr-2 text-[#D4A373]/80">- {cat.descriptionAr}</span>}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-[#00A859]/20 hover:bg-[#00A859] text-[#00A859] hover:text-white border border-[#00A859]/40 text-xs font-bold transition-all cursor-pointer flex items-center gap-1"
                      title="تعديل القسم"
                    >
